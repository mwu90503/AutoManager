// e.g. " [Practice: Limited]" - only Sleeper populates this sparsely
// (weekly practice DNP/Limited/Full), so it's appended when present
// rather than assumed.
function practiceSuffix(player) {
  return player.practice_participation ? ` [Practice: ${player.practice_participation}]` : '';
}
...
function zeroPointLines(roster) {
  return [
    ...
    ...(roster.criticalStatusStarters || []).map(
      (rec) =>
        `${rec.player.full_name} (${rec.player.team}) is ${rec.player.injury_status} in your ${rec.player.slot} slot.${practiceSuffix(rec.player)}`
    ),
...
function riskyLines(roster) {
  return (roster.riskyStatusStarters || []).map(
    (rec) => `${rec.player.full_name} (${rec.player.team}) is ${rec.player.injury_status}.${practiceSuffix(rec.player)}`
  );
}
...
function housekeepingLines(roster) {
  return [
    ...
    ...(roster.irRecommendations || []).map(
      (rec) =>
        `${rec.player.full_name} (${rec.player.injury_status}) on your ${rec.source === 'taxi' ? 'taxi squad' : 'bench'} could move to IR.`
    ),
...
function waiverLines(roster) {
  return (roster.waiverRecommendations || []).map(
    (rec) => `${rec.name} (${[rec.position, rec.team].filter(Boolean).join(' — ')}): ${rec.reason}`
  );
}

// variant: 'lineup' (zero-point + risky), 'housekeeping' (roster + taxi
// + waiver pickups, Tuesday only), or 'critical' (zero-point only - the
// daily "fix this" nag).
const VARIANT_SECTIONS = {
  ...
  housekeeping: [
    ...
    ['TAXI', taxiLines],
    ['WAIVER WIRE PICKUPS', waiverLines],
  ],
