import { supabase } from '@/lib/supabaseClient';
import { positionRank } from '@/lib/positionOrder';
import { byeWeekFor } from '@/lib/byeWeeks';
import { isReserveEligible } from '@/lib/injuryStatus';
import { eligiblePositionsForSlot } from '@/lib/flexEligibility';
import { getNflState } from '@/lib/sleeperClient';
import { getWeeklyProjections, projectedPoints } from '@/lib/sleeperProjections';
import { getUserByUsername, getRosters, getLeagueUsers } from '@/lib/sleeperClient';

// Doubtful means "almost certainly won't play" in practice, so it's
// treated as a hard swap alongside Out/IR/PUP, not a soft warning.
const SIT_STATUSES = ['Out', 'IR', 'PUP', 'Doubtful'];

// Everything the Recommendations UI (and the email digests) show for a
// given roster (players/starters/reserve/taxi arrays) in a league.
// `roster` can be a real stored row (own team) or an ephemeral one built
// live from Sleeper for a friend's view - this function never writes
// anything, so it's safe to call with either.
export async function analyzeRoster(league, roster) {
  const allIds = Array.from(
    new Set([...(roster.players || []), ...(roster.reserve || []), ...(roster.taxi || [])])
  );

  const { data: players, error: playersError } = await supabase
    .from('sleeper_players')
    .select('player_id, full_name, position, team, injury_status, practice_participation, years_exp')
    .in('player_id', allIds.length ? allIds : ['']);

  if (playersError) {
    throw new Error(playersError.message);
  }

  const playerById = new Map(players.map((p) => [p.player_id, p]));
  const resolve = (pid) =>
    playerById.get(pid) || {
      player_id: pid,
      full_name: pid,
      position: null,
      team: null,
      injury_status: null,
      practice_participation: null,
      years_exp: null,
    };

  const starterIds = roster.starters || [];
  const reserveIds = roster.reserve || [];
  const taxiIds = roster.taxi || [];
  const starterSet = new Set(starterIds);
  const reserveSet = new Set(reserveIds);
  const taxiSet = new Set(taxiIds);

  // Sleeper's `starters` array lines up index-for-index with the
  // non-bench/IR slots in the league's roster_positions, in the same
  // order Sleeper's own UI shows them (QB, RB, RB, WR, ...).
  const startingSlots = (league.roster_positions || []).filter(
    (p) => p !== 'BN' && p !== 'IR' && p !== 'TAXI'
  );
  // Sleeper represents an unfilled starting slot as "0"/empty rather than
  // omitting it - worth flagging since it's guaranteed zero points, not
  // just risky ones.
  const starters = starterIds.map((pid, i) => {
    const slot = startingSlots[i] || 'FLEX';
    if (!pid || pid === '0') {
      return { slot, player_id: `empty-${i}`, full_name: null, position: null, team: null, isEmpty: true };
    }
    return { slot, ...resolve(pid) };
  });

  const byPosition = (a, b) => positionRank(a.position) - positionRank(b.position);

  const bench = (roster.players || [])
    .filter((pid) => !starterSet.has(pid) && !reserveSet.has(pid) && !taxiSet.has(pid))
    .map(resolve)
    .sort(byPosition);

  const ir = reserveIds.map(resolve).sort(byPosition);
  const taxi = taxiIds.map(resolve).sort(byPosition);

  // Bench players eligible for a reserve/IR slot (IR/PUP always qualify;
  // other statuses like Out only count if the league's settings allow it)
  // that aren't actually on IR yet — the "you forgot to move this guy"
  // case. Checks the taxi squad too, not just the bench: a taxi player
  // who becomes IR/PUP-eligible should free up a taxi slot the same way
  // a bench player should free up a bench slot. IR slot count lives in
  // league.settings.reserve_slots, not roster_positions (same as
  // taxi_slots - neither is listed there).
  const irSlotsTotal = league.settings?.reserve_slots ?? 0;
  const irSlotsOpen = Math.max(irSlotsTotal - reserveIds.length, 0);
  const irRecommendations = [
    ...bench.map((p) => ({ player: p, source: 'bench' })),
    ...taxi.map((p) => ({ player: p, source: 'taxi' })),
  ]
    .filter(({ player }) => isReserveEligible(player.injury_status, league.settings))
    .map((rec) => ({ ...rec, irSlotsOpen }));

  // For each position with a recommendation, surface who else in the
  // league is unrostered at that spot. This is NOT a ranked "best
  // pickup" list (no projections data source is wired up) — just who's
  // actually available, alphabetically, so there's somewhere to start.
  const availableByPosition = {};
  if (irRecommendations.length) {
    const { data: leagueRosters } = await supabase
      .from('rosters')
      .select('players, reserve, taxi')
      .eq('league_id', league.id);

    const rosteredIds = new Set(
      (leagueRosters || []).flatMap((r) => [...(r.players || []), ...(r.reserve || []), ...(r.taxi || [])])
    );

    const positions = [...new Set(irRecommendations.map((r) => r.player.position).filter(Boolean))];
    for (const position of positions) {
      const { data: candidates } = await supabase
        .from('sleeper_players')
        .select('player_id, full_name, position, team, injury_status')
        .eq('position', position)
        .order('full_name');

      availableByPosition[position] = (candidates || [])
        .filter((c) => !rosteredIds.has(c.player_id) && !c.injury_status)
        .slice(0, 15);
    }
  }

  const nflState = await getNflState().catch(() => null);
  const isRegularSeason = nflState?.season_type === 'regular';

  // Fetched once, up front, so both the bench-replacement ranking below
  // and lineupSwapRecommendations share the same projections instead of
  // hitting the endpoint twice.
  let pointsFor = () => null;
  if (isRegularSeason && nflState?.week) {
    const positions = [...new Set([...starters, ...bench].map((p) => p.position).filter(Boolean))];
    try {
      const projections = await getWeeklyProjections(league.season, nflState.week, positions);
      pointsFor = (p) => projectedPoints(projections.get(p.player_id), league.scoring_settings);
    } catch {
      // Unofficial endpoint - fail quietly and fall back to unranked
      // (position-eligibility-only) bench suggestions.
    }
  }

  // Bench candidates for a compromised starter: eligible for that exact
  // slot (accounting for FLEX/SUPER_FLEX accepting multiple positions,
  // not just an exact position match) and not themselves sit-worthy or
  // on bye that week. Ranked so the most useful replacement leads: a
  // same-team backup (the direct handcuff/next-man-up, who inherits the
  // role directly) comes first, then by projected points - which already
  // bakes in matchup quality and expected role (a true WR1 in a plus
  // matchup projects higher than a WR3 in a tough one) without needing a
  // separate depth-chart or matchup data source.
  function findBenchReplacements(starter) {
    const eligible = eligiblePositionsForSlot(starter.slot);
    return bench
      .filter((b) => {
        if (!eligible.includes(b.position)) return false;
        if (SIT_STATUSES.includes(b.injury_status)) return false;
        if (isRegularSeason && byeWeekFor(b.team, league.season) === nflState.week) return false;
        return true;
      })
      .sort((a, b) => {
        const aSameTeam = a.team && a.team === starter.team ? 1 : 0;
        const bSameTeam = b.team && b.team === starter.team ? 1 : 0;
        if (aSameTeam !== bSameTeam) return bSameTeam - aSameTeam;

        const aPoints = pointsFor(a);
        const bPoints = pointsFor(b);
        if (aPoints == null && bPoints == null) return 0;
        if (aPoints == null) return 1;
        if (bPoints == null) return -1;
        return bPoints - aPoints;
      });
  }

  // Starters guaranteed (or near-guaranteed) to score zero this week: on
  // bye (this week or next, for advance notice), or ruled Out/IR/PUP/
  // Doubtful right now. Bye weeks only exist in the regular season, but
  // an injury status matters just as much in preseason/playoffs, so only
  // the bye check is gated on season type.
  const byeAlerts = [];
  const criticalStatusStarters = [];
  for (const p of starters) {
    if (p.isEmpty) continue;

    if (isRegularSeason) {
      const byeWeek = byeWeekFor(p.team, league.season);
      const weeksOut = byeWeek == null ? null : byeWeek - nflState.week;
      if (weeksOut != null && weeksOut >= 0 && weeksOut <= 1) {
        byeAlerts.push({ player: p, byeWeek, currentWeek: nflState.week, benchOptions: findBenchReplacements(p) });
        continue;
      }
    }

    if (SIT_STATUSES.includes(p.injury_status)) {
      criticalStatusStarters.push({ player: p, benchOptions: findBenchReplacements(p) });
    }
  }

  // Starters at real but non-guaranteed risk — a heads-up to double
  // check before kickoff, not a hard "swap them out" like the above.
  const riskyStatusStarters = starters
    .filter((p) => !p.isEmpty && p.injury_status === 'Questionable')
    .map((p) => ({ player: p, benchOptions: findBenchReplacements(p) }));

  // A healthy starter projected notably worse than a bench player who
  // could take their slot (FLEX/SUPER_FLEX-aware via findBenchReplacements).
  // Skips anyone already flagged above (empty/Out/IR/PUP/Doubtful/bye/
  // Questionable) — that's a different, already-covered problem.
  const lineupSwapRecommendations = [];
  if (isRegularSeason && nflState?.week) {
    const flaggedIds = new Set(
      [...criticalStatusStarters, ...byeAlerts, ...riskyStatusStarters].map((r) => r.player.player_id)
    );

    try {
      // Gather every (starter, bench) pair with a >4pt gap, then assign
      // greedily by biggest gap first so the same bench player never
      // gets suggested for more than one slot at once.
      const candidates = [];
      for (const starter of starters) {
        if (starter.isEmpty || flaggedIds.has(starter.player_id)) continue;
        const starterPoints = pointsFor(starter);
        if (starterPoints == null) continue;

        for (const b of findBenchReplacements(starter)) {
          const benchPoints = pointsFor(b);
          if (benchPoints == null) continue;
          const diff = benchPoints - starterPoints;
          if (diff > 4) candidates.push({ starter, starterPoints, bench: b, benchPoints, diff });
        }
      }
      candidates.sort((a, b) => b.diff - a.diff);

      const usedStarters = new Set();
      const usedBench = new Set();
      for (const c of candidates) {
        if (usedStarters.has(c.starter.player_id) || usedBench.has(c.bench.player_id)) continue;
        usedStarters.add(c.starter.player_id);
        usedBench.add(c.bench.player_id);
        lineupSwapRecommendations.push({
          starter: c.starter,
          starterPoints: c.starterPoints,
          bench: c.bench,
          benchPoints: c.benchPoints,
        });
      }
    } catch {
      // Unofficial endpoint - fail quietly rather than breaking the
      // whole roster analysis if it's ever down or changes shape.
    }
  }

  // Active roster (starters + bench, not IR/taxi) has room for a
  // free-agent pickup with no drop needed. Empty starter slots don't
  // count as "used" - they're already flagged separately and are just
  // as much open capacity as an empty bench spot.
  const activeSlotsTotal = (league.roster_positions || []).filter((p) => p !== 'IR' && p !== 'TAXI').length;
  const filledStarters = starters.filter((s) => !s.isEmpty).length;
  const activeSlotsUsed = filledStarters + bench.length;
  const openBenchSlots = Math.max(activeSlotsTotal - activeSlotsUsed, 0);

  // Bench players still young enough for taxi (Sleeper's taxi_years
  // setting) that aren't parked there yet - wasting a bench spot on a
  // stash that doesn't need one. Excludes anyone with an injury status:
  // taxi is for healthy unproven players, not an alternative to IR - a
  // hurt rookie should go on IR (or stay put), not taxi.
  const taxiYears = league.settings?.taxi_years;
  const taxiSlotsTotal = league.settings?.taxi_slots ?? 0;
  const taxiSlotsOpen = Math.max(taxiSlotsTotal - taxiIds.length, 0);
  const taxiRecommendations =
    taxiSlotsTotal > 0 && taxiYears != null
      ? bench
          .filter((p) => p.years_exp != null && p.years_exp <= taxiYears && !p.injury_status)
          .map((p) => ({ player: p, taxiSlotsOpen }))
      : [];

  const emptyStarterSlots = starters
    .filter((s) => s.isEmpty)
    .map((s) => ({ ...s, benchOptions: findBenchReplacements(s) }));

  // Players sitting on IR whose injury_status has cleared - they're
  // occupying a reserve slot for no reason and could come back to the
  // active roster.
  const healthyOnIr = ir
    .filter((p) => !p.injury_status)
    .map((p) => ({ player: p, hasBenchRoom: openBenchSlots > 0 }));

  return {
    league,
    roster: {
      ...roster,
      starters,
      bench,
      ir,
      taxi,
      irRecommendations,
      availableByPosition,
      byeAlerts,
      criticalStatusStarters,
      riskyStatusStarters,
      lineupSwapRecommendations,
      openBenchSlots,
      taxiRecommendations,
      emptyStarterSlots,
      healthyOnIr,
    },
  };
}

// The normal path: your own stored roster for this league.
export async function analyzeLeagueRoster(league) {
  const { data: roster, error: rosterError } = await supabase
    .from('rosters')
    .select('*')
    .eq('league_id', league.id)
    .eq('is_own_team', true)
    .single();

  if (rosterError) {
    throw new Error(rosterError.message);
  }

  return analyzeRoster(league, roster);
}

// A one-off, read-only snapshot of someone else's roster in this same
// league, fetched live from Sleeper - nothing is written to Supabase, so
// this can never overwrite who "own team" actually is for this league.
// Accepts either a typed `username` (resolved via Sleeper) or a known
// `ownerId` (skips that lookup - used by the Opponents list, which
// already has each team's sleeper_owner_id from our own DB).
export async function analyzeFriendRoster(league, { username, ownerId, teamName } = {}) {
  let targetOwnerId = ownerId;
  let label = teamName;

  if (!targetOwnerId) {
    const user = await getUserByUsername(username);
    if (!user) {
      throw new Error('No Sleeper user with that username');
    }
    targetOwnerId = user.user_id;
    label = label || username;
  }

  const [rosters, leagueUsers] = await Promise.all([
    getRosters(league.sleeper_league_id),
    getLeagueUsers(league.sleeper_league_id),
  ]);

  const friendRoster = rosters.find((r) => r.owner_id === targetOwnerId);
  if (!friendRoster) {
    throw new Error(`${label || 'That user'} is not in this league`);
  }

  const teamNameByOwnerId = new Map(
    leagueUsers.map((u) => [u.user_id, u.metadata?.team_name || u.display_name])
  );

  return analyzeRoster(league, {
    team_name: teamNameByOwnerId.get(targetOwnerId) || label,
    players: friendRoster.players || [],
    starters: friendRoster.starters || [],
    reserve: friendRoster.reserve || [],
    taxi: friendRoster.taxi || [],
    wins: friendRoster.settings?.wins || 0,
    losses: friendRoster.settings?.losses || 0,
    ties: friendRoster.settings?.ties || 0,
  });
}

// Analyzes every imported league, skipping ones with no real roster
// (e.g. an inactive league that was never actually drafted) so they
// don't spam empty-slot alerts for a league nobody's playing.
export async function getActiveLeagueAnalyses() {
  const { data: leagues, error } = await supabase.from('leagues').select('*');
  if (error) {
    throw new Error(error.message);
  }

  const results = [];
  for (const league of leagues) {
    try {
      const result = await analyzeLeagueRoster(league);
      if ((result.roster.players || []).length > 0) {
        results.push(result);
      }
    } catch {
      // No roster row yet, or a transient fetch error - skip it rather
      // than failing the whole batch.
    }
  }
  return results;
}

// Groups analyzed leagues by who imported them, so scheduled digests
// can send each person their own email instead of blending everyone's
// leagues into one.
export function groupResultsByUser(leagueResults) {
  const byUser = new Map();
  for (const result of leagueResults) {
    const username = result.league.imported_by_username;
    if (!byUser.has(username)) byUser.set(username, []);
    byUser.get(username).push(result);
  }
  return byUser;
}
