...
export async function analyzeFriendRoster(league, sleeperUsername) {
  ...
  const friendRoster = rosters.find((r) => r.owner_id === user.user_id);
  if (!friendRoster) {
    throw new Error(`${sleeperUsername} is not in this league`);
  }
  ...
  return analyzeRoster(league, {
    team_name: teamNameByOwnerId.get(user.user_id) || sleeperUsername,
