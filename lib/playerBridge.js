...
// Resolves one external player to our Sleeper player_id: team defenses
// first (Sleeper represents a D/ST as the team abbreviation itself,
// e.g. "SEA" - name matching "Seahawks D/ST" would never hit that),
// then espn_id (sparse but exact when present), then name match,
// disambiguated by team/position if the name alone is ambiguous (e.g.
// two "Josh Allen"s across the league - the QB and the Jaguars DE).
export function bridgePlayer(index, { espnId, name, team, position }) {
  if (position === 'DEF' && team) return team;
