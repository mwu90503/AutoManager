...
import { braveNewsSearch } from './braveSearch';
...
Each result includes how long ago it was published - weigh the most recent ones most heavily, since practice status changes day to day.
...
export async function getPlayerOsint({ name, team, position }) {
  ...
  const results = await braveNewsSearch(query, 8);
  ...
  const sourcesText = results
    .map((r, i) => {
      const snippets = r.extraSnippets.length ? r.extraSnippets.join(' ') : r.description;
      return `[${i + 1}] (${r.age || 'unknown age'}) ${r.title}\n${snippets}\nURL: ${r.url}`;
    })
    .join('\n\n');
