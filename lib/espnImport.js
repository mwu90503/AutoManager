...
import { encrypt } from '@/lib/cookieEncryption';
...
export async function importEspnLeague({ leagueId, season, username, espnS2, swid }) {
  const league = await getEspnLeague(leagueId, season, { espnS2, swid });
    ...
    .upsert(
      {
        ...
        // Saved (encrypted) so the daily refresh cron can keep this
        // league current without asking again every time - see
        // lib/cookieEncryption.js for the security model.
        espn_s2_encrypted: encrypt(espnS2),
        espn_swid_encrypted: encrypt(swid),
      },
